...
// Target Surplus Pasokan Bersih (dikurangi 2 unit per bangunan)
...
const computeBuildingCounts = (pop) => {
  ...
  for (const k of olahanKeys) {
    ...
    const baseCount = Math.max(10, Math.ceil(targetProd / BASE_PRODUKSI[k]));
    // KURANGI 2 UNTUK MASING-MASING BANGUNAN (minimal 0)
    olahanCount[k] = Math.max(0, baseCount - 2);
  }
  ...
  for (const k of peternakanKeys) {
    ...
    const baseCount = Math.max(10, Math.ceil(targetProd / BASE_PRODUKSI[k]));
    // KURANGI 2 UNTUK MASING-MASING BANGUNAN (minimal 0)
    peternakanCount[k] = Math.max(0, baseCount - 2);
  }
  ...
  for (const k of agrikulturKeys) {
    ...
    const baseCount = Math.max(10, Math.ceil(targetProd / BASE_PRODUKSI[k]));
    // KURANGI 2 UNTUK MASING-MASING BANGUNAN (minimal 0)
    agrikulturCount[k] = Math.max(0, baseCount - 2);
  }
  ...
  for (const k of perikananKeys) {
    ...
    const baseCount = Math.max(10, Math.ceil(targetProd / BASE_PRODUKSI[k]));
    // KURANGI 2 UNTUK MASING-MASING BANGUNAN (minimal 0)
    perikananCount[k] = Math.max(0, baseCount - 2);
  }
  ...
  for (let line of lines) {
    ...
    if (inValues && line.trim().startsWith('(')) {
      ...
      if (match) {
        ...
        const newVals = columns.map(col => countsMap[col] !== undefined ? countsMap[col] : 8);
