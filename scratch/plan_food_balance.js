...
// Target Surplus Pasokan Bersih (Produksi - Konsumsi Pop = +100 sampai +300 unit)
...
// Simulasi Konsumsi Bahan Baku Industri Olahan Pangan
// Kebutuhan industri:
// beras -> padi (10)
// gula -> tebu (10)
// roti -> gandum (15)
// mie_instan -> gandum (20)
// minyak_goreng -> kelapa_sawit (12)
// susu -> sapi_perah (8)
// pengolahan_daging -> ayam_unggas (5), sapi_potong (2), domba_kambing (3)

const computeBuildingCounts = (pop) => {
  // A. Hitung dulu Olahan Pangan
  ...
  for (const k of olahanKeys) {
    ...
    olahanCount[k] = Math.ceil(targetProd / BASE_PRODUKSI[k]);
  }

  // Hitung Konsumsi Pabrik Olahan Pangan terhadap Bahan Baku Mentah
  ...
  for (const k of peternakanKeys) {
    ...
    peternakanCount[k] = Math.ceil(targetProd / BASE_PRODUKSI[k]);
  }
  ...
  for (const k of agrikulturKeys) {
    ...
    const facCons = rawFactoryCons[k] || 0;
    const targetProd = popCons + facCons + TARGET_SURPLUS;
    agrikulturCount[k] = Math.ceil(targetProd / BASE_PRODUKSI[k]);
  }
...
const idn = profiles.find(p => p.country_slug === 'indonesia');
if (idn) {
  const res = computeBuildingCounts(idn.jumlah_penduduk);
  console.log(`\n=== KALKULASI PRESISI RANTAI PASOK INDONESIA (Populasi: ${idn.jumlah_penduduk.toLocaleString()}) ===`);
  
  console.log("\n--- SEKTOR PETERNAKAN (Jumlah Bangunan Terhitung) ---");
  Object.keys(res.peternakanCount).forEach(k => {
    const popCons = (idn.jumlah_penduduk / 1000) * FOOD_CONSUMPTION_PER_CAPITA[k];
    const facCons = res.rawFactoryCons[k] || 0;
    const bCount = res.peternakanCount[k];
    const grossProd = bCount * BASE_PRODUKSI[k];
    const netSaldo = grossProd - popCons - facCons;
    console.log(`${k.padEnd(15)}: JmlBangunan=${bCount.toString().padStart(5)}, GrossProd=${grossProd.toLocaleString().padStart(10)}, PopCons=${popCons.toFixed(1).padStart(10)}, FacCons=${facCons.toString().padStart(6)}, NetSaldo=+${netSaldo.toFixed(1)}`);
  });

  console.log("\n--- SEKTOR OLAHAN PANGAN (Jumlah Bangunan Terhitung) ---");
  Object.keys(res.olahanCount).forEach(k => {
    const popCons = (idn.jumlah_penduduk / 1000) * FOOD_CONSUMPTION_PER_CAPITA[k];
    const bCount = res.olahanCount[k];
    const grossProd = bCount * BASE_PRODUKSI[k];
    const netSaldo = grossProd - popCons;
    console.log(`${k.padEnd(18)}: JmlBangunan=${bCount.toString().padStart(5)}, GrossProd=${grossProd.toLocaleString().padStart(10)}, PopCons=${popCons.toFixed(1).padStart(10)}, NetSaldo=+${netSaldo.toFixed(1)}`);
  });
}
