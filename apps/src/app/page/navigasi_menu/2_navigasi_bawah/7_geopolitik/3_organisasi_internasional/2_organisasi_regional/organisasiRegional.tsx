    ...
    <div className="w-full h-full flex flex-col bg-[#0F2424]">
      ...
      <div className="px-8 py-5 border-b border-[#00FFAA]/20 flex items-center justify-between bg-[#0A1A1A] flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-bold text-[#E0E0E0] uppercase tracking-wide">
              {orgName}
            </h3>
      ...
      <div className="flex-1 overflow-y-auto p-6 sm:p-8 bg-[#0F2424] custom-scrollbar">
        <div className="w-full bg-[#0A1A1A] border border-[#00FFAA]/20 rounded-xl p-6 shadow-lg">
          <div className="flex justify-between items-center mb-4 border-b border-[#00FFAA]/15 pb-2">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-10 gap-2 text-[#6B8A8A] font-bold">
              <Loader2 className="h-5 w-5 animate-spin text-[#00FFAA]" />
              Memuat data anggota...
            </div>
          ) : members.length === 0 ? (
            <div className="py-10 text-center text-[#6B8A8A] font-medium">
                  ...
                  <div
                    ...
                    className={`rounded-xl p-3 flex flex-col gap-1 shadow-sm border cursor-pointer transition-all ${
                      isPlayer
                        ...
                        : 'bg-[#051111] border-[#00FFAA]/20 hover:border-[#00FFAA]/50 text-[#E0E0E0]'
                    }`}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-sm font-bold ${isPlayer ? 'text-[#00FFAA]' : 'text-[#E0E0E0]'}`}>
                          {member.country}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <ChevronRight className="h-4 w-4 text-[#6B8A8A] group-hover:text-[#00FFAA] transition-colors" />
                      </div>
                    </div>
                    <span className={`text-[10px] font-bold uppercase tracking-wider self-start px-2 py-0.5 rounded-full ${
                      member.status === 'Anggota Tetap' ? 'bg-amber-500/15 text-amber-400 border border-amber-500/30' :
                      member.status === 'Anggota' ? 'bg-[#00FFAA]/15 text-[#00FFAA] border border-[#00FFAA]/30' :
                      'bg-[#00FFAA]/10 text-[#6B8A8A] border border-[#00FFAA]/20'
                    }`}>
